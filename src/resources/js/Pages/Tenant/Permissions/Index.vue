...
import { Plus, Pencil, Trash2, KeyRound, X } from 'lucide-vue-next';
...
const deletePermission = (permission) => {
    if (confirm(`Tem certeza que deseja remover a permissão "${permission.name}"?`)) {
            ...
            <div class="content-container w-full max-w-5xl">
                ...
                <div class="flex flex-col md:flex-row items-center justify-between gap-4 p-6 border-b-dynamic">
                    <div>
                        ...
                        <p class="form-subtitle">Crie e gerencie as permissões que podem ser atribuídas aos papéis.</p>
                    </div>
                ...
                <div class="p-4 md:p-6">
                    <div v-if="permissions.data.length > 0" class="space-y-4">
                        <div v-for="permission in permissions.data" :key="permission.id" class="role-card">
                            <div class="flex-1">
                                <p class="role-name">{{ permission.name }}</p>
                            </div>
                            <div class="flex items-center space-x-2 ml-4">
                                <button @click="editPermission(permission)" class="table-action-btn hover:text-amber-600" title="Editar"><Pencil class="w-5 h-5" /></button>
                                <button @click="deletePermission(permission)" class="table-action-btn hover:text-red-600" title="Excluir"><Trash2 class="w-5 h-5" /></button>
                            </div>
                        </div>
                    </div>
                     <div v-else class="text-center py-10">
                        <p class="text-gray-500 dark:text-gray-400">Nenhuma permissão encontrada.</p>
                    </div>
                </div>
                                ...
                                <form @submit.prevent="submit">
                                    <div class="p-6">
                                        ...
                                        <div class="mt-6">
                                            ...
                                            <input type="text" v-model="form.name" id="name" class="form-input" placeholder="Ex: gerenciar relatorios" required>
                                            <p class="text-xs text-gray-500 dark:text-gray-400 mt-1 text-left">Use o formato "ação recurso" (ex: ver usuarios, criar servicos).</p>
