...
const confirmingConvenioDeletion = ref(false);
const convenioToDelete = ref(null);
const deleteForm = useForm({});

const confirmConvenioDeletion = (convenio) => {
    convenioToDelete.value = convenio;
    confirmingConvenioDeletion.value = true;
};

const deleteConvenio = () => {
    deleteForm.delete(route('admin.convenios.destroy', convenioToDelete.value.id), {
        preserveScroll: true,
        onSuccess: () => {
            confirmingConvenioDeletion.value = false;
            convenioToDelete.value = null;
        }
    });
};

const deleteConfirmationMessage = computed(() => {
    return convenioToDelete.value ? `Tem certeza que deseja remover o convênio "${convenioToDelete.value.objeto}"? Esta ação não pode ser desfeita.` : '';
});
...
<template>
    ...
    <TenantLayout title="Gestão de Convênios">
            ...
            <div class="max-w-7xl mx-auto">
                <div class="content-container">
                    ...
                    <div class="p-4 md:p-6">
                        <div v-if="convenios.data.length > 0">
                             <ul class="divide-y divide-gray-200 dark:divide-white/10">
                                <li v-for="convenio in convenios.data" :key="convenio.id" class="convenio-item group">
                                    <div class="flex-1 min-w-0">
                                        <p class="convenio-title">{{ convenio.objeto }}</p>
                                        <p class="convenio-entity">{{ convenio.entidade.nome }}</p>
                                        <div class="mt-3 flex flex-wrap items-center gap-x-4 gap-y-2">
                                           <span :class="statusClass(convenio.status)" class="badge-base">
                                                {{ convenio.status }}
                                           </span>
                                           <span class="text-xs text-gray-500 dark:text-gray-400">
                                               Vigência: {{ formatDate(convenio.data_inicio) }} a {{ formatDate(convenio.data_fim) }}
                                           </span>
                                        </div>
                                    </div>
                                    <div class="flex items-center space-x-2 ml-4 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <button @click="editConvenio(convenio)" class="table-action-btn hover:text-amber-600 dark:hover:text-yellow-400" title="Editar"><Pencil class="w-5 h-5" /></button>
                                        <button @click="confirmConvenioDeletion(convenio)" class="table-action-btn hover:text-red-600 dark:hover:text-red-400" title="Excluir"><Trash2 class="w-5 h-5" /></button>
                                    </div>
                                </li>
                            </ul>
                        </div>
                        <div v-else class="empty-state">
                            <Handshake class="w-12 h-12 text-gray-400 dark:text-gray-500" />
                            <h3 class="mt-2 text-lg font-medium text-gray-900 dark:text-white">Nenhum convênio registrado</h3>
                            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Comece adicionando um novo convênio.</p>
                        </div>
                    </div>

                    <div v-if="convenios.data.length > 0" class="px-6 pb-4 border-t border-gray-200 dark:border-white/10 pt-4">
                                    ...
                                    <div class="p-6">
                                        ...
                                        <div class="mt-6 space-y-8">
                                            <fieldset class="space-y-6">
                                                <legend class="section-title">Detalhes do Convênio</legend>
                                                <div>
                                            ...
                                            <fieldset class="space-y-6">
                                                <legend class="section-title">Vigência e Status</legend>
                                            ...
                                            <fieldset>
                                                <legend class="section-title">Observações</legend>
                                                 <div class="mt-4">
                                                    <label for="descricao" class="form-label">Descrição / Detalhes Adicionais</label>
        ...
        <ConfirmationModal
            :show="confirmingConvenioDeletion"
            title="Excluir Convênio"
            :message="deleteConfirmationMessage"
            @close="confirmingConvenioDeletion = false"
            @confirm="deleteConvenio"
            danger
        />
    </TenantLayout>
</template>
...
.convenio-item { @apply flex items-center justify-between p-4; }
.convenio-title { @apply text-lg font-bold text-emerald-800 dark:text-emerald-300; }
.convenio-entity { @apply text-sm font-medium text-gray-600 dark:text-gray-300; }
