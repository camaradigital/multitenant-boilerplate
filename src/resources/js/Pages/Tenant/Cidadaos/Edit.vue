...
const submit = () => {
    form.put(route('admin.cidadaos.update', { cidadao: form.id }), {
        onSuccess: () => router.visit(route('admin.cidadaos.index')),
        preserveScroll: true,
    });
};
...
const validateField = async (fieldName, value) => {
    ...
    try {
        await axios.post(route('realtime.validate'), { field: fieldName, value: value, id: props.cidadao.id });
    } catch (error) {
...
<template>
    <Head title="Editar Cidadão" />
    <TenantLayout title="Editar Cidadão">
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Editar Cidadão</h2>
        </template>
...
.input-valid { @apply border-emerald-500 focus:border-emerald-500 focus:ring-emerald-500; @apply dark:border-green-500 dark:focus:border-green-500 dark:focus:ring-green-500; }
.input-invalid { @apply border-red-500 focus:border-red-500 focus:ring-red-500; @apply dark:border-red-400 dark:focus:border-red-400 dark:focus:ring-red-400; }
</style>
