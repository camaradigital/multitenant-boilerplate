...
const confirmingEntidadeDeletion = ref(false);
const entidadeToDelete = ref(null);
const deleteForm = useForm({});

const confirmEntidadeDeletion = (entidade) => {
    entidadeToDelete.value = entidade;
    confirmingEntidadeDeletion.value = true;
};

const deleteEntidade = () => {
    deleteForm.delete(route('admin.entidades.destroy', entidadeToDelete.value.id), {
        preserveScroll: true,
        onSuccess: () => {
            confirmingEntidadeDeletion.value = false;
            entidadeToDelete.value = null;
        }
    });
};

const deleteConfirmationMessage = computed(() => {
    return entidadeToDelete.value ? `Tem certeza que deseja remover a entidade "${entidadeToDelete.value.nome}"? Esta ação não pode ser desfeita.` : '';
});
...
<template>
    ...
    <TenantLayout title="Entidades e Convênios">
            ...
            <div class="max-w-7xl mx-auto">
                <div class="content-container">
                    ...
                    <div class="p-4 md:p-6">
                        <div v-if="entidades.data.length > 0">
                            <ul class="divide-y divide-gray-200 dark:divide-white/10">
                                <li v-for="entidade in entidades.data" :key="entidade.id" class="entity-item group">
                                     <div class="flex items-center space-x-4 flex-1 min-w-0">
                                        <div class="entity-avatar">
                                            <span>{{ getInitials(entidade.nome) }}</span>
                                        </div>
                                        <div class="flex-1 min-w-0">
                                            <div class="flex items-center gap-2">
                                                <p class="entity-name">{{ entidade.nome }}</p>
                                                <div :class="entidade.is_active ? 'bg-green-500' : 'bg-gray-400'" class="status-dot" :title="entidade.is_active ? 'Ativa' : 'Inativa'"></div>
                                            </div>
                                            <p class="entity-category">{{ entidade.categoria }}</p>
                                        </div>
                                    </div>
                                    <div class="hidden md:flex items-center space-x-4">
                                         <div class="text-right">
                                             <p class="entity-contact">{{ entidade.email }}</p>
                                             <p class="entity-contact">{{ entidade.telefone }}</p>
                                         </div>
                                        <div class="flex items-center space-x-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                            <button @click="editEntidade(entidade)" class="table-action-btn hover:text-amber-600 dark:hover:text-yellow-400" title="Editar"><Pencil class="w-5 h-5" /></button>
                                            <button @click="confirmEntidadeDeletion(entidade)" class="table-action-btn hover:text-red-600 dark:hover:text-red-400" title="Excluir"><Trash2 class="w-5 h-5" /></button>
                                        </div>
                                    </div>
                        ...
                        <div v-else class="empty-state">
                            <Library class="w-12 h-12 text-gray-400 dark:text-gray-500" />
                            <h3 class="mt-2 text-lg font-medium text-gray-900 dark:text-white">Nenhuma entidade registrada</h3>
                            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Comece adicionando a primeira entidade parceira.</p>
                        </div>
                    </div>

                    <div v-if="entidades.data.length > 0" class="px-6 pb-4 border-t border-gray-200 dark:border-white/10 pt-4">
                                    ...
                                    <div class="p-6">
                                        ...
                                        <div class="mt-6 space-y-8">
                                            <fieldset class="space-y-6">
                                                <legend class="section-title">Dados da Entidade</legend>
                                            ...
                                            <fieldset class="space-y-6">
                                                ...
                                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                                    <div>
                                                        ...
                                                        <input id="telefone" v-model="form.telefone" type="text" class="form-input" :class="{ 'input-invalid': realtimeErrors.telefone || form.errors.telefone, 'input-valid': !realtimeErrors.telefone && !form.errors.telefone && form.telefone }" v-maska data-maska="['(##) ####-####', '(##) #####-####']" placeholder="(00) 0000-0000" />
                                                    ...
                                                    <div>
                                                        ...
                                                        <input id="email" v-model="form.email" type="email" class="form-input" :class="{ 'input-invalid': realtimeErrors.email || form.errors.email, 'input-valid': !realtimeErrors.email && !form.errors.email && form.email }" />
                                            ...
                                            <fieldset>
                                                ...
                                                <div class="mt-4">
                                                    <label for="is_active" class="toggle-switch-label">
                                                        <span class="font-medium text-gray-900 dark:text-gray-100">Status da Entidade</span>
        ...
        <ConfirmationModal
            :show="confirmingEntidadeDeletion"
            title="Excluir Entidade"
            :message="deleteConfirmationMessage"
            @close="confirmingEntidadeDeletion = false"
            @confirm="deleteEntidade"
            danger
        />
    </TenantLayout>
</template>
...
.entity-item { @apply flex items-center justify-between p-4; }
.entity-avatar { @apply h-10 w-10 flex-shrink-0 rounded-full flex items-center justify-center font-bold; @apply bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-300; }
.entity-name { @apply text-base font-bold text-emerald-800 dark:text-emerald-300 truncate; }
.entity-category { @apply text-sm text-gray-500 dark:text-gray-400 truncate; }
.entity-contact { @apply text-sm text-gray-600 dark:text-gray-400 truncate;}
.status-dot { @apply h-2.5 w-2.5 rounded-full flex-shrink-0; }
